...
export default function CardTile({ card, onEdit, onDelete }) {
  const used = card.limit > 0 ? Math.min(100, Math.round((card.currentInvoice / card.limit) * 100)) : 0;
  ...
  return (
    ...
    >
      ...
      <div className="card-tile-progress">
        ...
        <small>{formatCurrency(card.currentInvoice)} de {formatCurrency(card.limit)} de limite</small>
      </div>

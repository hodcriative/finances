...
function serialize(row) {
  return {
    ...
    notifyIncomeExpenseAlerts: Boolean(row.notify_income_expense_alerts),
  };
}
...
router.patch("/", (req, res) => {
  ...
  const { name, theme, notifyBudgetAlerts, notifyGoalAlerts, notifyIncomeExpenseAlerts } = req.body || {};
  const next = {
    ...
    notify_income_expense_alerts:
      notifyIncomeExpenseAlerts !== undefined
        ? (notifyIncomeExpenseAlerts ? 1 : 0)
        : current.notify_income_expense_alerts,
  };

  db.prepare(
    "UPDATE preferences SET name = ?, theme = ?, notify_budget_alerts = ?, notify_goal_alerts = ?, notify_income_expense_alerts = ? WHERE user_id = ?"
  ).run(
    next.name,
    next.theme,
    next.notify_budget_alerts,
    next.notify_goal_alerts,
    next.notify_income_expense_alerts,
    req.userId
  );
